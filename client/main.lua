...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("QBCore:Notify", "Vous faites déjà quelque chose", "error")
        end
    else
        TriggerEvent("QBCore:Notify", "Vous ne pouvez pas faire cette action", "error")
    end
end
        ...
        while runProgThread do
            if isDoingAction then
                ...
                if not isProp and Action.prop ~= nil and Action.prop.model ~= nil then
                    QBCore.Functions.RequestModel(Action.prop.model)
                    ...
                    if not isPropTwo and Action.propTwo ~= nil and Action.propTwo.model ~= nil then
                        QBCore.Functions.RequestModel(Action.propTwo.model)

                        local pCoords = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
                        local modelSpawn2 = CreateObject(GetHashKey(Action.propTwo.model), pCoords.x, pCoords.y, pCoords.z, true, true, true)

                        local netid = ObjToNet(modelSpawn2)
                        SetNetworkIdExistsOnAllMachines(netid, true)
                        NetworkSetNetworkIdDynamic(netid, true)
                        SetNetworkIdCanMigrate(netid, false)
